...
def add_trade():
    with open(FILENAME, "a", newline="") as f:
        ...
        pair = input("Pair (BTCUSD/XAUUSD/etc): ").upper()
        r = self_float("R Multiple: ")
...
# LOAD JOURNAL DATA
# =========================
def load_journal_data(filter_pair=None):
    R = []
    ...
    with open(FILENAME) as f:
        ...
        for row in reader:
          if filter_pair and row["pair"].upper()  != filter_pair:
            continue
...
def calculate_metrics(R):
    ...
    metrics = {
        ...
        "profit_factor": wins.sum() / abs(losses.sum()) if losses.sum() != 0
        else float("inf"),
    ...
    metrics["best_trade"] = R.max() if len(R) else 0
    metrics["worst_trade"] = R.min() if len(R) else 0
...
# MAIN ANALYSIS
# =========================
def analyze_journal(filter_pair=None):
    R, pairs, monthly, emotions = load_journal_data(filter_pair)
...
def main():
    ...
    while True:
        ...
        if choice == "1":
          ...
          pair_filter = input("Filter pair (enter = all): ").upper()

          if pair_filter == "":
            analyze_journal()
          else:
            analyze_journal(pair_filter)
